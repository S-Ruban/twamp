...
{	
	...
	server_max(int port) throws IOException
	{
		sdf.setTimeZone(java.util.TimeZone.getTimeZone("GMT+5:30")); 
		server = new ServerSocket(port);
		ConcurrentLinkedQueue<Thread> strings = new ConcurrentLinkedQueue<Thread>();
		...
		while(true)
		{
			...
			try
			{
				...
				Thread t = new Thread(new Runnable()
				{
					...
					int src_port, dest_port, seq, ack, HLEN, win_size, checksum, urg_ptr, temp, test_seq, packets_received, base_seq, base_ack ;
					...
					synchronized int calculate_checksum()
					{
						int cs = 0 ;
					...
					synchronized void start(int p)
					{
						try
						{
							twamp.server = server_max.server ;
							System.out.println("Waiting for client.");
							twamp.socket = server_max.socket ;
							System.out.println("Client is online.\n\n");
							twamp.in = server_max.in ;
							twamp.out = server_max.out ;
							while(true)
							{
					...
					synchronized void reflect() throws IOException
					{
						int c = 0 ;
						while(true)
						{
...
//							sst_frac = bytearr_to_long(client_test_packet, 16, 19);
					...
					synchronized void extract(byte msg[])
					{
				...
				//			sst_frac = bytearr_to_long(request_session_message, 92, 95);
							server_max.date = new Date((twamp.bytearr_to_long(request_session_message, 88, 91)-twamp.OFFSET)*1000+(twamp.bytearr_to_long(request_session_message, 92, 95)/(long)1e6));
				...
				strings.add(t);				// add current thread to the queue
				strings.peek().start();		// initiate current thread 
				strings.peek().join();
				strings.poll();				// remove thread from queue after closing TWAMP and TCP connection
			}
			catch(Exception e)
			{
				e.printStackTrace();
